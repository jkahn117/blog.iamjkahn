...
- `pnpm format` — Format supported files with Oxfmt and Astro files with Prettier
- `pnpm format:check` — Check formatting without writing files
- `pnpm lint` — Lint JavaScript, TypeScript, React, and Astro scripts with Oxlint
...
**Formatting and linting**: Oxfmt is the primary formatter and Oxlint is the linter. Prettier remains scoped to `.astro` files because Oxfmt does not support Astro templates yet. Both formatters use semicolons, double quotes, and two-space indentation. Configuration lives in `.oxfmtrc.json`, `.oxlintrc.json`, and `.prettierrc.json`.
